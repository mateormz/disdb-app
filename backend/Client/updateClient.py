...
def lambda_handler(event, context):
    try:
        ...
        if authenticated_role == 'distributor' and client['PK'] != authenticated_pk:
            print("[WARNING] Distributor is trying to access unauthorized client data")
            return {
                'statusCode': 403,
                'headers': {'Content-Type': 'application/json'},
            ...
            print("[WARNING] Delivery person is trying to access unauthorized client data")
            return {
                'statusCode': 403,
                'headers': {'Content-Type': 'application/json'},
                'body': json.dumps({'error': 'Unauthorized - You can only access your own clients'})
        ...
        print("[INFO] Updating client in DynamoDB")
        ...
        print("[INFO] Client updated successfully")
        return {
            'statusCode': 200,
            'headers': {'Content-Type': 'application/json'},
