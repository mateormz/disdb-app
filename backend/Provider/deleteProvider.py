...
def lambda_handler(event, context):
    try:
        ...
        # Query DynamoDB to ensure the provider exists
        ...
        if provider['PK'] != authenticated_pk:
            print("[WARNING] User is trying to delete unauthorized provider data")
            return {
                'statusCode': 403,
                'headers': {'Content-Type': 'application/json'},
                'body': json.dumps({'error': 'Unauthorized - You can only delete your own providers'})
            }

        # Delete provider from DynamoDB
        print(f"[INFO] Deleting provider from DynamoDB with PK={pk} and SK={sk}")
        delete_response = provider_table.delete_item(
            Key={'PK': pk, 'SK': sk}
        )
        print(f"[DEBUG] DynamoDB delete_item response: {delete_response}")

        print("[INFO] Provider deleted successfully")
        return {
            'statusCode': 200,
            'headers': {'Content-Type': 'application/json'},
            'body': json.dumps({'message': 'Provider deleted successfully'})
        }
