...
def lambda_handler(event, context):
    try:
        ...
        if authenticated_role != 'distributor':
            print("[WARNING] Unauthorized access - Only distributors can access this function")
            return {
                'statusCode': 403,
                'headers': {'Content-Type': 'application/json'},
                'body': json.dumps({'error': 'Unauthorized - Only distributors can access providers'})
            }
        ...
        # Ensure the authenticated user matches the distributor
        if pk != authenticated_pk:
            print("[WARNING] Unauthorized access - Cannot access another distributor's providers")
            return {
                'statusCode': 403,
                'headers': {'Content-Type': 'application/json'},
        ...
        print("[INFO] Querying DynamoDB for providers")
